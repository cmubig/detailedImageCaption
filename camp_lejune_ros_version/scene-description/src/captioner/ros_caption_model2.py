...
class model(object):
    def __init__(self, dim_feature=[64, 1536], dim_embed=300, dim_hidden=512, n_time_step=19):
        ...
        self.pretrained_model = source_dir+'/model2/model_max-95960'#source_dir+'/model2/model_max'# 
        ...
        self.load_scopes_model = 'word_embedding, image_features, lstm_init, MIL, attribute_attention_layer,a_lstm,v_lstm,m_lstm, s_lstm, decode_layer, attention_pivot, color_module,size_module,count_module,spatial_module,semantic_module,attention_module'
    ...
    def _init_image_feature(self,mean_feature,reuse=False):
        with tf.variable_scope('lstm_init',reuse=reuse):
            ...
            init2 = tf.layers.dense(mean_feature,self.LSTM,activation=tf.nn.relu,name='lstm_init2') #(N,3*LSTM)
    ...
    def color_module(self,pivot,h,dim,is_dropout=True,reuse=True):
        with tf.variable_scope('color_module', reuse=reuse): 
            ...
            l = tf.layers.dense(l, dim, activation=tf.nn.relu, name='h_cl2')
        return l
    ...
    def size_module(self,pivot,h,dim,is_dropout=True,reuse=True):
        with tf.variable_scope('size_module', reuse=reuse):
            ...
            l =  tf.layers.dense(l,dim, activation=tf.nn.relu, name='h_sz2') # N x 512tf.truediv
        return l
    ...
    def count_module(self,pivot,h,dim,is_dropout=True,reuse=True):
        with tf.variable_scope('count_module', reuse=reuse):
            ...
            l = tf.layers.dense(l, dim, activation=tf.nn.relu, name='h_ct2')
        return l
    ...
    def build_sampler(self,max_len=20):
        ...
        lstm_init,lstm_init2 = self._init_image_feature(mean_feature,reuse=False)
        
        MIL_embedding = tf.expand_dims(self._word_embedding(self.MIL_list,reuse = False),0) #(1,N,616)
        
        lstm_cell_a = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.LSTM)
        (c_a,h_a) = lstm_cell_a.zero_state(batch_size=batch_size, dtype=tf.float32)
        ...
        lstm_cell_m = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.LSTM)
        (c_m,h_m) = lstm_cell_m.zero_state(batch_size=batch_size, dtype=tf.float32)

        lstm_cell_s = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.LSTM)
        (c_s,h_s) = lstm_cell_s.zero_state(batch_size=batch_size, dtype=tf.float32)

        final_prob,mean_activation,final_prob2 = self.MIL_classifier(region_feature,mean_feature)
        final_prob2_gated = self._attribute_gate_layer(h_s,mean_proj,final_prob2,reuse=False)
        MIL_attribute,_ = self._attribute_vector_op(batch_size,final_prob2,MIL_embedding)

        with tf.variable_scope('a_lstm', reuse=False):
            _, (c_a, h_a) = lstm_cell_a(inputs=tf.concat((lstm_init,MIL_attribute),axis=1), state= (c_a, h_a))

        with tf.variable_scope('v_lstm', reuse=False):
            _, (c_v, h_v) = lstm_cell_v(inputs=tf.concat((lstm_init,MIL_attribute),axis=1), state=(c_v, h_v))

        with tf.variable_scope('m_lstm', reuse=False):
            _, (c_m, h_m) = lstm_cell_m(inputs=tf.concat((lstm_init2,MIL_attribute),axis=1), state=(c_m, h_m))

        with tf.variable_scope('s_lstm', reuse=False):
            _, (c_s, h_s) = lstm_cell_s(inputs=tf.concat((lstm_init2,MIL_attribute),axis=1), state=(c_s, h_s))

        _ = self. _decode_layer(tf.concat([h_s],1),reuse=False) 
        ...
        for t in range(self.T):
            ...
            x_a = tf.concat([h_s,mean_proj,previous_word],1)
            with tf.variable_scope('a_lstm', reuse=True):
                _, (c_a, h_a) = lstm_cell_a(inputs=x_a, state=(c_a, h_a))

            pivot,alpha_pivot = self._attention_pivot(region_proj,h_a,reuse=(t!=0)) #(N,H) (N,R)

            final_prob2_gated = self._attribute_gate_layer(h_a,pivot,final_prob2,reuse=True)
            MIL_attribute,_ = self._attribute_vector_op(batch_size,final_prob2_gated,MIL_embedding)


            x_v = tf.concat([pivot,h_a,MIL_attribute],1) #(2*LSTM)
            with tf.variable_scope('v_lstm', reuse=True):
                _, (c_v, h_v) = lstm_cell_v(inputs=x_v, state=(c_v, h_v))
            logits = self._decode_layer_copy(h_v)
            init_sample = tf.argmax(tf.nn.softmax(logits), 1)
            init_est = self._word_embedding(inputs=init_sample, reuse=True)

            

            with tf.variable_scope('m_lstm', reuse=True):
                _, (c_m, h_m) = lstm_cell_m(inputs=tf.concat((pivot,init_est),axis=1), state=(c_m, h_m))

            h_cl = self.color_module(pivot,h_m,self.LSTM,reuse = (t!=0)) #(N,H)
            h_sz = self.size_module(pivot,h_m,self.LSTM,reuse = (t!=0)) #(N,H)
            h_ct = self.count_module(pivot,h_m,self.LSTM,reuse = (t!=0)) #(N,H)
            h_sp = self.spatial_module(pivot,h_m,self.LSTM,reuse = (t!=0)) #(N,H)
            h_se = self.semantic_module(pivot,h_m,self.LSTM,reuse = (t!=0)) #(N,H)
                
            color_logits = self._decode_layer_copy(h_cl) #(N,V)
            color_max = tf.argmax(color_logits, 1)
            color_attribute = self._word_embedding(inputs=color_max,reuse = True)

            count_logits = self._decode_layer_copy(h_ct) #(N,V)
            count_max = tf.argmax(count_logits, 1)
            count_attribute = self._word_embedding(inputs=count_max,reuse = True)

            size_logits = self._decode_layer_copy(h_sz) #(N,V)
            size_max = tf.argmax(size_logits, 1)
            size_attribute = self._word_embedding(inputs=size_max,reuse = True)

            spatial_logits = self._decode_layer_copy(h_sp) #(N,V)
            spatial_max = tf.argmax(spatial_logits, 1)
            spatial_attribute = self._word_embedding(inputs=spatial_max,reuse = True)
        
            semantic_logits = self._decode_layer_copy(h_se) #(N,V)
            semantic_max = tf.argmax(semantic_logits, 1)
            semantic_attribute = self._word_embedding(inputs=semantic_max,reuse = True)

            module_attribute = tf.stack([color_attribute,count_attribute,size_attribute,spatial_attribute,semantic_attribute],1)

            h_mod,alpha_logits,alpha_m,beta_m = self._attention_module(module_attribute,init_est,h_m,reuse=(t!=0)) #(N,D) (N,num_mod) (N,1)
            x_s = tf.concat([h_v,h_mod],1)
            with tf.variable_scope('s_lstm', reuse=True):
                _, (c_s, h_s) = lstm_cell_s(inputs=x_s, state=(c_s, h_s))
            logits2 = self. _decode_layer(h_s,reuse=True)            
